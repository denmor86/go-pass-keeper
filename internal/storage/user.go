...
func (s *UserStorage) Get(ctx context.Context, email string, password string) (*models.User, error) {
	const query = `
...
`
	...
	err := s.db.Pool.QueryRow(ctx, query, email, password).Scan(&user.ID, &user.Email)
	if err != nil {
